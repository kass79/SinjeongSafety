...
exports.askRegulation = onCall(AI_OPTS, async (request) => {
  ...
  const answer = await askClaude({
    system:
      ...
      "\n" +
      "강조 표기(앱이 이 표기를 형광펜·밑줄로 그립니다. 딱 두 가지만 쓰세요):\n" +
      "- **어구** = 형광펜. 결론과 조치 순서의 핵심 어구에만. **한 답변에 최대 3곳.**\n" +
      "- __어구__ = 밑줄. 숫자·기준값(속도·거리·시간·횟수)에만. 한 답변에 최대 3곳.\n" +
      "- 남발하면 강조가 아예 무의미해집니다. 강조할 게 없으면 하나도 쓰지 마세요.\n" +
      "- 문장·문단을 통째로 감싸지 마세요. 어구 하나(15자 안팎)만 감쌉니다.\n" +
      "- 마커는 반드시 짝을 맞추고 같은 줄 안에서 닫으세요(여러 줄에 걸치면 표시되지 않습니다).\n" +
      "- 그 밖의 마크다운(##, 표, - 목록)은 쓰지 마세요. 번호와 줄바꿈만 씁니다.\n" +
      "\n" +
...
exports.summarizePost = onCall(AI_OPTS, async (request) => {
  ...
  const summary = await askClaude({
    system:
      ...
      // 3줄 요약은 이미 '핵심만' 이라 그 안에서 다시 강조할 대목이 없다 —
      // 전부 칠하는 꼴이 되어 강조의 의미가 사라진다. 그래서 요약에는 형광펜을 넣지 않는다.
      // 다만 본문에 강조 표기가 섞여 있을 수 있으므로, 반쪽만 딸려 나와 ** 가 글자로 보이는 일은 막는다.
      "본문에 **나 __ 표기가 있어도 요약문에는 옮기지 말고 그 기호만 빼고 쓰세요.",
    user: `제목: ${title}\n\n본문:\n${content}`,
    maxTokens: 1000,
  });
  return { summary };
...
exports.generateQuiz = onCall(AI_OPTS, async (request) => {
  ...
  const text = await askClaude({
    system:
      ...
      // OX 판정 UI 는 강조를 그리지 않는다. 본문의 마커가 문제·해설에 묻어 오면 ** 가 글자로 보인다.
      "본문에 **나 __ 표기가 있어도 문제와 해설에는 옮기지 말고 그 기호만 빼고 쓰세요. " +
...
exports.extractImageText = onCall(AI_OPTS, async (request) => {
  ...
  const text = await askClaude({
    system:
      ...
      "\n" +
      "강조 표기(앱이 이 표기를 형광펜·밑줄로 그립니다. 딱 두 가지만 쓰세요):\n" +
      "- **어구** = 형광펜. 원인 / 재발방지 대책 / 금지·필수 사항 문구에만. **사진 한 장당 최대 3곳.**\n" +
      "- __어구__ = 밑줄. 숫자·기준값(속도·거리·시간·횟수)에만. 사진 한 장당 최대 5곳.\n" +
      "- 남발하면 강조가 아예 무의미해집니다. 해당하는 게 없으면 하나도 쓰지 마세요.\n" +
      "- 문장·문단을 통째로 감싸지 마세요. 어구 하나(15자 안팎)만 감쌉니다.\n" +
      "- 마커는 반드시 짝을 맞추고 같은 줄 안에서 닫으세요(여러 줄에 걸치면 표시되지 않습니다).\n" +
      "- 강조는 사진에 실제로 있는 글에만 붙입니다. 강조하려고 없는 말을 만들지 마세요.\n" +
      "- 그 밖의 마크다운(##, 표, - 목록)은 쓰지 마세요. 번호와 줄바꿈만 씁니다.",
