 ...
 *
 * 인터넷·API를 전혀 쓰지 않는다(터널 대비). assets/regulations.json 912조문을
 * 그대로 훑어서 점수를 매긴다. 900여 개면 색인 없이도 한 번에 10~30ms라 충분히 빠르다.
 *
...
object RegulationSearch {
    ...
    private val BOOK_WEIGHT = mapOf(
        "전동차승무원업무예규" to 1.15,
        "운전취급규정" to 1.10,
        "운전취급세부요령" to 1.10,      // 취급규정의 현장판 — 급할 때 찾는 것
        "운전관계직원업무내규" to 1.05,
        "차량기지운전취급내규" to 1.05,
        "승무원지도운용내규" to 1.00,
        "운전무사고성적심사규정" to 0.95, // 심사·행정 — 현장 긴급도 낮음
        "인사규정" to 1.00,
        "취업규칙" to 1.00
    )

    /** 배지에 쓸 짧은 이름 (좁은 화면에서 넘치지 않게) */
    private val SHORT_NAME = mapOf(
        "전동차승무원업무예규" to "승무원업무예규",
        "운전무사고성적심사규정" to "무사고심사규정",
        "차량기지운전취급내규" to "차량기지내규",
        "운전관계직원업무내규" to "관계직원내규"
    )

    fun shortBookName(name: String): String = SHORT_NAME[name] ?: name
