...
- 규정 데이터: `assets/regulations.json` **9권 912조문** (키 n/t/b, 책이름→조문배열). 오프라인 동작이
  ...
  - **고치지 못한 구간**(경계 근거 없음 — 추측 금지): 운전취급규정 11개 조문의 속도·신호 표 숫자 덩어리
    (제102·103·105·182·185·190·197·278·282·347·364조, 예: 곡선 제한속도 28자리를 13칸으로 못 나눔 —
    잘못 끊으면 기관사가 틀린 제한속도를 읽는다), 예규 제105조 무공백 문장, 인사규정 부칙 승진경로표.
    인사규정 색인 70·71은 변환기가 조문을 잘못 쪼갠 것(본문이 쉼표 하나)이라 병합이 필요 — 조문 수가
    바뀌는 작업이므로 별도로.
